...
class Stripe extends msPaymentHandler implements msPaymentInterface
{
	...
	function __construct(xPDOObject $object, $config = array())
	{
		...
		$paymentUrl = $siteUrl . substr($assetsUrl, 1) . 'payment/stripe.php';

		$this->config = array_merge(array(
			'paymentUrl' => $paymentUrl,
			'apiUrl' => $this->modx->getOption('mspstripe_api_url', null, 'https://api-3t.paypal.com/nvp'),
			'checkoutUrl' => $this->modx->getOption('ms2_payment_paypal_checkout_url', null,
				'https://www.paypal.com/webscr?cmd=_express-checkout&token='
			),
			'currency' => $this->modx->getOption('mspstripe_api_currency', null, 'eur'),
			'user' => $this->modx->getOption('mspstripe_api_pb'),
			'json_response' => false,
		), $config);
	}
